...
- (void) _gl_draw {
    ...
    if(frm) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, frm->frm_width, frm->frm_height, 0, GL_RED, GL_UNSIGNED_BYTE, frm->frm_bmp->data[0]);
        glBindTexture(GL_TEXTURE_2D, _textures[1]);
        glPixelStorei(GL_UNPACK_ROW_LENGTH, frm->frm_bmp->linesize[1]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, frm->frm_width / 2, frm->frm_height / 2, 0, GL_RED, GL_UNSIGNED_BYTE, frm->frm_bmp->data[1]);
        glBindTexture(GL_TEXTURE_2D, _textures[2]);
        glPixelStorei(GL_UNPACK_ROW_LENGTH, frm->frm_bmp->linesize[2]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, frm->frm_width / 2, frm->frm_height / 2, 0, GL_RED, GL_UNSIGNED_BYTE, frm->frm_bmp->data[2]);
    }
